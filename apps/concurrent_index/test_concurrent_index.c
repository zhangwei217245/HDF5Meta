...
#include "utils/cbase/c99_stdlib.h"
...
void *doQuery(void *tp) {
    ...
    for (c = 0; c < num_kvs; c++) {
        if (c % tparam->num_threads == tparam->tid) {
            ...
            power_search_rst_t *rst = exact_metadata_search(meta_attr->attr_name, meta_attr->attribute_value, meta_attr->attr_type);
            if(rst->size==0){
                // printf("Not found for type INTEGER with value %d", *value);
            }
            resultCount += rst->size;
            num_queried++;
        }
    }
    timer_pause(tparam->timerWatch);
